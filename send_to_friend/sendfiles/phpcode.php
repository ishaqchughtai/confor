	...
	if($url && count($friends) && isset($_REQUEST['your_email']) && strpos($_REQUEST['your_email'],'@')){ // bad validation, meh
		...
		foreach($friends as $friend){
			...
			$replace = array(
				...
				'IMAGE' => ((isset($_REQUEST['image_url']) && $_REQUEST['image_url']) ? '<a href="'.$url.'" target="_blank"><img src="'.$_REQUEST['image_url'].'" border="0" alt="Image from your friend"></a>' : ''),
			);
		...
		if(preg_match_all('#content-type:\s?(.*)#i',$data,$matches)){
			...
			foreach(array(
				...
				if(!preg_match('#'.$check_content.'#i',$content_type)){
					return array(
						'Content type of "'.$content_type.'" not supported by program.',
					);
				}
			}
		}else{
			return array(
				"Content type for URL not provided, please check URL and try again.",
			);
		}
