...
//  $this->load->library('uri');
  ...
  $keywords_shop = get_info_for_adv();

  //if($this->uri->segment(6)) echo 'Keyword: '.print_r($keywords_shop);
  //Array ( [video_blog_keyword] => peur avion commandant Piché [speaker_name] => Robert Piché ) Keyword: 1

  if($keywords_shop && is_array($keywords_shop)) 

  {

      $arr_shop_info = $this->mshopproduct->get_relate_shop($keywords_shop,$lang_shop_shop,base_url().'/prestashop/img/p/');

	  //if($this->uri->segment(6)) {echo '<br>Keyword: ';print_r($arr_shop_info);};
