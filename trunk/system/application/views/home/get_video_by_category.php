...
<?php foreach($video_cate as $rows):?>			  
    ...
    <div class="content_item">
        ...
        <h5><?php echo __("CF_by")?> 
            <a alt="<?php echo $rows['first_name'].' '.$rows['name'] ?>" title="<?php echo $rows['first_name'].' '.$rows['name'] ?>" href="<?php echo site_url('video/speaker').'/'.$rows['mem_id'].'/5'?>"><b><?php echo $rows['first_name'].' '.$rows['name'] ?></b></a> 
            <?php echo __("CF_in")?> : 
    ...
    <p>
        <a alt="<?php echo $rows['title'].' - '.$rows['first_name'].' '.$rows['name']?>" title="<?php echo $rows['title'].' - '.$rows['first_name'].' '.$rows['name']?>" href="<?php echo site_url('video/play').'/'.$rows['vid_id']?>"><img alt="<?php echo $rows['title'].' - '.$rows['first_name'].' '.$rows['name']?>" title="<?php echo $rows['title'].' - '.$rows['first_name'].' '.$rows['name']?>" src="<?php echo base_url().'thumbs/'.$rows['shash']?>" width="160" height="90" class="alignleft" /></a>
    </p>
